...
public class ControllerFactory {
    ...
    public void initialize(Set<Class<?>> c, ServletContext ctx){
        ...
        for(Class<?> clazz : c){
            try {
                ...
                String name = r.method().name();
                String[] values = r.value();
                for (String value: values) {
                    beanMap.put(name +"-"+ value,clazz.getConstructor().newInstance());
                }
    ...
    private Object getBean(String key){
        ...
        if(!beanMap.containsKey(key))
            throw new ControllerNotFoundException("not found key");
